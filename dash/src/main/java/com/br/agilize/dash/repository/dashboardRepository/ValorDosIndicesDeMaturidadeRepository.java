...
public interface ValorDosIndicesDeMaturidadeRepository extends JpaRepository<ValorDosIndicesDeMaturidadeEntity, Long> {
    ...
    @Query("SELECT v FROM ValorDosIndicesDeMaturidadeEntity v WHERE v.maturidade.esteira.id = :esteiraId AND v.itemDeMaturidade.tipoMaturidade = :tipoMaturidade ORDER BY v.dataHoraValor DESC")
